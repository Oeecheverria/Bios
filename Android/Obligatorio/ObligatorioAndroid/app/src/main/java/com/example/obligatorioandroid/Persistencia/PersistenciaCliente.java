...
class PersistenciaCliente implements IPersistenciaCliente {
    ...
    public List<DTCliente> listarClientes() throws ExcepcionPersonalizada {
        ...
        Cursor datosParticular = null;
        Cursor datosComercial = null;
        ...
        try {
            ...
            while (datosPadre.moveToNext()) {
                datosParticular = baseDatos.query(BD.PARTICULARES, BD.Particulares.COLUMNAS, null, null, null, null, BD.Particulares.IDCLIENTE);
                while (datosParticular.moveToNext())
                    listaClientes.add(instanciarClienteParticular(datosPadre, datosParticular));
                datosComercial = baseDatos.query(BD.COMERCIALES, BD.Comerciales.COLUMNAS, null, null, null, null, BD.Comerciales.IDCLIENTE);
                while (datosComercial.moveToNext())
                    listaClientes.add(instanciarClienteComercial(datosPadre, datosComercial));
            }
            ...
            if (datosParticular != null) datosParticular.close();
            if (datosComercial != null) datosComercial.close();
    ...
    public void modificarCliente(DTCliente cliente) throws ExcepcionPersonalizada {

        {
            ...
            try {
                ...
                if (cliente instanceof DTParticular) {
                    valoresHijo.put(BD.Particulares.NOMBRECOMPLETO, ((DTParticular) cliente).getNombreCompleto());
                    baseDatos.update(BD.PARTICULARES, valoresCliente, BD.Particulares.IDCLIENTE + " = ?", new String[]{String.valueOf(cliente.getId())});
                } else {
                    valoresHijo.put(BD.Comerciales.RAZONSOCIAL, ((DTComercial) cliente).getRazonSocial());
                    baseDatos.update(BD.COMERCIALES, valoresCliente, BD.Comerciales.IDCLIENTE + " = ?", new String[]{String.valueOf(cliente.getId())});
                }
